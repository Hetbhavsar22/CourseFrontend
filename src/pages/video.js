...
function Video() {
  ...
  const validateForm = () => {
    ...
    if (selectedOption === "video") {
      ...
      if (!doc) {
        errors.doc = "Valid DOC file is required.";
      }
      if (!ppt) {
  ...
  const moveVideo = (videoId, direction) => {
    ...
    const courseVideos = videos.filter(video => video.courseId === videoCourseId);
  
    const courseVideoIndex = courseVideos.findIndex(video => video._id === videoId);
    if (courseVideoIndex === -1) return;
  
    const targetIndex = direction === 'up' ? courseVideoIndex - 1 : courseVideoIndex + 1;
    ...
    [courseVideos[courseVideoIndex].order, courseVideos[targetIndex].order] = 
      [courseVideos[targetIndex].order, courseVideos[courseVideoIndex].order];
    ...
    const updatedOrder = courseVideos.map((video, index) => ({ _id: video._id, order: index }));
    axios.post('http://localhost:8080/video/updateVideoOrder', { videos: updatedOrder })
